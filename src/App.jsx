...
export default function Dashboard() {
  ...
  // ════════════════════════════════════════════════════════════════
// FEATURE 01 — FORECAST / PROJECTION
// Uses linear regression on existing monthly data to project
// remaining months of the current FY with a dashed purple bar.
// ════════════════════════════════════════════════════════════════

// ── STEP 1: Add this utility function at the top of your file ───
function linReg(ys) {
  const n = ys.length;
  if (n < 2) return { predict: () => ys[0] || 0 };
...
// ── STEP 2: Add state toggle at the top of your Dashboard() ─────
const [showForecast, setShowForecast] = useState(true);

// ── STEP 3: Add this derived data computation (needs monthlyData) ─
// Place this inside Dashboard(), after monthlyData is computed.
const MONTHS_ORD = ["Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec","Jan","Feb","Mar"];
...
const forecastData = useMemo(() => {
  ...
  // Fit regression to existing qty and value series
  const regQty = linReg(monthlyData.map(d => d.qty));
  const regVal = linReg(monthlyData.map(d => d.value));
  ...
  const forecast = remaining.map((m, i) => ({
    ...
    qty:         null,
    value:       null,
    forecastQty: regQty.predict(monthlyData.length + i),
    forecastVal: regVal.predict(monthlyData.length + i),
  }));

  // Stitch last actual point into first forecast so bars connect visually
  const last = monthlyData[monthlyData.length - 1];
  return [
    ...monthlyData.map((d, i) => ({
      ...d,
      forecastQty: i === monthlyData.length - 1 ? d.qty  : null,
      forecastVal: i === monthlyData.length - 1 ? d.value: null,
    })),
    ...forecast,
  ];
}, [monthlyData, showForecast]);

// ── STEP 4: Toggle button — drop anywhere in your UI ────────────
<button
  onClick={() => setShowForecast(s => !s)}
  style={{
    background: showForecast ? "rgba(139,92,246,0.15)" : "rgba(255,255,255,0.04)",
    border:     `1px solid ${showForecast ? "rgba(139,92,246,0.4)" : "rgba(255,255,255,0.1)"}`,
    color:      showForecast ? "#8B5CF6" : "#64748B",
...
// ── STEP 5: Chart — replace your existing monthly ComposedChart ──
...
<ResponsiveContainer width="100%" height={270}>
  <ComposedChart data={forecastData}>
    ...
    <Bar yAxisId="l" dataKey="qty" name="Actual Qty"
      fill="#F59E0B" fillOpacity={0.75} radius={[4,4,0,0]} maxBarSize={32} />

    {/* Forecast quantity bars — purple, semi-transparent */}
    {showForecast && (
      <Bar yAxisId="l" dataKey="forecastQty" name="Forecast Qty"
        fill="#8B5CF6" fillOpacity={0.45} radius={[4,4,0,0]} maxBarSize={32} />
    )}

    {/* Forecast value line — dashed */}
    {showForecast && (
      <Line yAxisId="r" dataKey="forecastVal" name="Forecast Value"
...
// ── STEP 6: Summary chips below the chart ────────────────────────
{showForecast && forecastData.some(d => d.forecastQty && !d.qty) && (
  <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
    {forecastData
      .filter(d => d.forecastQty && !d.qty)
      .map((d, i) => (
        <span key={i} style={{
          ...
          {d.month}: ~{d.forecastQty >= 1000 ? `${(d.forecastQty/1000).toFixed(1)}K MT` : `${d.forecastQty} MT`}
        </span>
      ))}
  </div>
)}
